...
const Table = () => {
  ...
  const transactionsFiltered = transactions.filter(t => t.category !== 'Income')
  let totalExpenses = []
  transactionsFiltered.map(e => totalExpenses.push(Number.parseFloat(e.amount)))
  totalExpenses = totalExpenses.reduce((acc, curr) => acc + curr, 0)

  const incomeFiltered = transactions.filter(t => t.category === 'Income')
  let totalIncome = []
  incomeFiltered.map(e => totalIncome.push(Number.parseFloat(e.amount)))
  totalIncome = totalIncome.reduce((acc,curr) => acc + curr, 0)

  ///uprzątnę i opakuję w funkcje
  ...
  return (
    ...
    <div className={css.tableContainer}>
      ...
      <ul className={css.tableList}>
        {transactionsFiltered.map(({amount, category}, idx) => {
         return (
